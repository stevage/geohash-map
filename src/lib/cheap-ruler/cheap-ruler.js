...
export default class CheapRuler {
    ...
    constructor(lat, units) {
        ...
        if (units && !factors[units])
            throw new Error(
                `Unknown unit ${units}. Use one of: ${Object.keys(factors).join(
                    ', '
                )}`
            );
    ...
    pointToSegmentDistance(p, a, b) {
        ...
        if (dx !== 0 || dy !== 0) {
            const t =
                (wrap(p[0] - x) * this.kx * dx + (p[1] - y) * this.ky * dy) /
                (dx * dx + dy * dy);
    ...
    lineSlice(start, stop, line) {
        ...
        if (!equals(line[l], slice[0]) && l <= r) slice.push(line[l]);

        for (let i = l + 1; i <= r; i++) {
            slice.push(line[i]);
        }

        if (!equals(line[r], p2.point)) slice.push(p2.point);
