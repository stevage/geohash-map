...
function toMultiLineString(lineStrings) {
  return turf.multiLineString(lineStrings.map((ls) => ls.geometry.coordinates))
}

function getCutRegions(map, level) {
  const grats = toMultiLineString(map.querySourceFeatures('graticules').map(toLineString))
...
export function initMappingRegions(map) {
  ...
  // map.U.addLineLayer('regions-full-line', 'regions-full', {
  //     lineColor: 'white',
  // });
  return
  map.U.addLineLayer('regions-full-line', 'composite', {
    sourceLayer: 'admin',
  ...
  EventBus.$on('moveend', () => {
    ...
    map.U.addLineLayer('regions-line', 'regions', {
      lineColor: 'black',
      lineWidth: 3,
    })
  })
