...
function makeStreaks(hashesFC) {
  ...
  function makeStreaks() {
    ...
    for (const hash of hashes) {
      ...
      for (const participant of hash.properties.participants) {
        ...
        streak.allSuccesses = streak.allSuccesses && hash.properties.success
      }
    }
    cleanup(1e9)
    return streaks
  ...
  return {
    ...
    features: makeStreaks().map((streak) => ({
      ...
      geometry: {
        type: 'LineString',
        coordinates: streak.hashes.map((hash) => hash.geometry.coordinates),
      },
    })),
  }
}
...
export function updateStreakStyle({ map, filters }) {
  ...
  if (first) {
    ...
    map.U.addLineLayer('streaks-under', 'streaks', {
      lineColor: '#333',
      // lineDasharray: [2, 2],
      lineOpacity: 0.9,
    ...
    map.U.addLineLayer('streaks-line', 'streaks', {
