...
async function loadGeohashes(map: mapU) {
  ...
  const startDate = window.app.overrideTime ? new Date(window.app.overrideTime) : new Date()
    ...
    for (const lngSign of [-1, 1]) {
      for (const latSign of [-1, 1]) {
        for (let lng = 0; lng <= 179; lng++) {
          const whichHash = lng < 30 || lngSign === 1 ? hash['east'] : hash['west']
          ...
          for (let lat = 0; lat <= 85; lat++) {
            features.push(
              makeHash(
                [lngSign * (lng + whichHash?.lng), latSign * (lat + whichHash?.lat)],
...
async function makeBushBash(map: mapU) {
  ...
  if (map.getZoom() > 13) {
    ...
    if (
      lastHash &&
      lastHash.geometry.coordinates[0] === hashes[0].geometry.coordinates[0] &&
      lastHash.geometry.coordinates[1] === hashes[0].geometry.coordinates[1]
    ) {
      return
    }
    ...
    if (distances[0]) {
      const bushBashStart = turf.nearestPointOnLine(distances[0][0], hash)
...
export function updateGeohashes(map: mapU) {
  ...
  if (first) {
    ...
    map.U.addCircleLayer('geohashes-circle', 'geohashes', {
      minzoom: 5,
      visibility: 'none',
      circleColor: 'black',
    ...
    map.U.addSymbolLayer('geohashes-label', 'geohashes', {
      minzoom: 8,
      visibility: 'none',
    ...
    map.U.addLineLayer('hashRing-underline', 'hashRing', {
      lineColor: 'hsl(50,30%,18%)',
    })
    map.U.addLineLayer('hashRing-line', 'hashRing', {
      lineColor: 'hsl(50,50%,70%)',
      lineDasharray: [4, 50],
    })
    map.U.addSymbolLayer('hashRing-label', 'hashRing', {
      textField: ['get', 'name'],
    ...
    map.U.addLineLayer('bushBash-line', 'bushBash', {
      lineColor: 'hsl(50,50%,70%)',
      lineDasharray: [2, 2],
    })
    map.U.addSymbolLayer('bushBash-label', 'bushBash', {
