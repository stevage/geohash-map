...
export async function initGTG(map: mapU) {
    ...
    map.U.addFill('countries-gtg-fill', 'countries-gtg', {
        fillColor: 'hsla(150,100%,80%,0.05)',
    });
    map.U.addLine('countries-gtg-line', 'countries-gtg', {
        lineColor: 'hsla(150,100%,80%,0.2)',
        lineWidth: 1,
    });
    const countries = await fetch('gtg/countries.geojson').then((r) =>
        r.json()
    );
    const countriesShifted = turf.featureCollection(
        (
            turf.flatten(countries) as unknown as FeatureCollection<Polygon>
        ).features.map((f: Feature<Polygon>) => translateFeature(f, 180, 0))
    );
...
function onlyBorders() {
    for (let y = 0; y < 30; y++)
        for (let x = 0; x < 180; x++) {
            const pixelBounds = [
                window.map.project([x, y]),
                window.map.project([x + 1, y + 1]),
            ];
            ...
            if (features.length) {
                ...
                if (
                    features.find((f: any) =>
                        f.properties.iso_3166_1.match(/(^MM-|-MM$)/)
                    )
                ) {
...
window.z.reloadGtg = () => {
    ...
    for (let y = 30; y < 90; y++)
        for (let x = 0; x < 180; x++) gtg(y, x, 'out', false);
    ...
    for (let y = 0; y < 30; y++)
        for (let x = 0; x < 180; x++)
            for (let i = -2; i <= 2; i++)
                for (let j = -2; j <= 2; j++)
                    if (window.graticulesById[`${y + j},${x + i}`])
                        gtg(y, x, 'out', false);
    ...
    '21,93 27,91 22,102 19,104 29,61, 29,60'
        .split(' ')
