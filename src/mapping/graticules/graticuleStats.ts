...
export function makeGraticuleStats({
  ...
  for (const expedition of hashes.features) {
    ...
    expeditionsByGraticule[`${y},${x}`] = expeditionsByGraticule[`${y},${x}`] || []
    ...
    if (expedition.properties.success) {
      ...
      if (!g.firstParticipants) {
        g.firstParticipants = expedition.properties.participants.join('\n')
        g.firstParticipantsOrMultiple =
          expedition.properties.participants.length > 1 ? 'multiple' : g.firstParticipants
      }
      ...
      g.lastParticipantsOrMultiple =
        expedition.properties.participants.length > 1 ? 'multiple' : g.lastParticipants
      g.history += 'Y' //'✅';
    } else {
      g.failures++
      if (!g.successes) {
    ...
    g.firstExpeditionDays = Math.min(g.firstExpeditionDays || 0, expedition.properties.days)
    g.lastExpeditionDays = expedition.properties.days
    g.daysSinceExpedition = dateToDays(new Date()) - expedition.properties.days
    ...
    for (const p of expedition.properties.participants) {
      ...
      if (expedition.properties.success) {
        g.participantsSuccesses[p] = (g.participantsSuccesses[p] || 0) + 1
      }
    }
  ...
  for (const [, g] of Object.entries(graticulesById)) {
    ...
    g.mostSuccessfulParticipantCount = successes.sort((a, b) => b[1] - a[1])[0][1]
    g.mostSuccessfulParticipants = successes
      .filter(([, count]) => count === g.mostSuccessfulParticipantCount)
      .map(([id]) => id)
      .join('\n')
    g.mostSuccessfulParticipantsOrMultiple =
      successes.filter(([p, count]) => count === g.mostSuccessfulParticipantCount).length > 1
        ? 'multiple'
