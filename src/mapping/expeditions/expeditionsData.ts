...
function enrichExpeditions(expeditions: FeatureCollection<Point>) {
  ...
  expeditions.features.forEach((f, i) => {
    ...
    exp.weekDayName = 'Sunday Monday Tuesday Wednesday Thursday Friday Saturday'.split(' ')[
      exp.weekday
    ]
    ...
    exp.graticuleNameShort = gname && gname.match(/[a-z() ], [a-z]/i) ? gname.split(', ')[0] : gname
    exp.graticuleCountry = gname && gname.match(/[a-z], [a-z]/i) ? gname.split(', ')[1] : ''
    ...
    exp.hardship = achievements.includes('One_with_Nature_geohash_achievement')
      ? 'One with nature'
  ...
  expeditions.features.forEach((f) => {
    ...
    exp.participantsOrMultiple =
      exp.participants.length > 1 ? 'Multiple' : exp.participants[0] || 'Unknown'
    exp.participantsStringLower = exp.participantsString.toLowerCase()
    exp.participantsCount = exp.participants.length
    const expeditions = exp.participants.map((p: string) => participants[p].expeditions)
    ...
    exp.experienceTotal = expeditions.length
      ...
      : 0
    const days = exp.participants.map((p: string) => exp.days - participants[p].firstExpeditionDays)
    exp.experienceDaysMax = days.length ? Math.max(...days) : 0
    exp.experienceDaysMin = days.length ? Math.min(...days) : 0
    exp.experienceDaysTotal = days.length ? days.reduce((a: number, b: number) => a + b, 0) : 0
  })
  return expeditions
}

export async function getExpeditions(local: boolean): Promise<FeatureCollection<Point>> {
